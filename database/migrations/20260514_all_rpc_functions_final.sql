-- ═══════════════════════════════════════════════════════════════════════════════
-- ALL RPC FUNCTIONS — Dashboard Intelligence (Phases 1 & 2)
-- Fixes applied: visitor_logs, checkin_time, no activity_log, no member enum
-- ═══════════════════════════════════════════════════════════════════════════════

-- ─── 1. PROPERTY HEALTH SCORE ─────────────────────────────────────────────────

create or replace function
get_property_health_score(p_property_id uuid)
returns jsonb
language plpgsql
security definer
stable
...
begin
  ...
  v_breakdown := jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'weight', 0.5,
      'impact', round((100.0 - v_sop_pct) * 0.5, 1)
    ),
    jsonb_build_object(
      ...
      'impact', case
        ...
        then round((v_avg_res_hours - 48.0) * 0.5, 1)
        else 0.0
      end
    ),
    jsonb_build_object(
...
create or replace function
get_ticket_funnel(p_property_id uuid, p_days int default 30)
returns table (
  status_label text,
  ticket_count bigint,
  avg_hours numeric
...
create or replace function
get_attention_items(p_property_id uuid, p_limit int default 10)
returns table (
  id uuid,
  severity text,
  type text,
...
as $$
  select * from (
    select
      ...
      (
        'Ticket #' || coalesce(t.ticket_number, t.id::text)
        || ' "' || left(t.title, 40)
        || '" will breach SLA in '
        || round(extract(epoch from (t.sla_deadline - now())) / 60, 0)
        || ' minutes'
      )::text as description,
    ...
    select
      ...
      (
        ...
        || ' "' || left(t.title, 40)
        || '" has been open for '
        || round(extract(epoch from (now() - t.created_at)) / 86400, 0)
        || ' days'
      )::text as description,
      'ticket'::text as entity_type,
      t.id as entity_id,
...
-- ─── 4. RECENT ACTIVITY ───────────────────────────────────────────────────────
-- Uses visitor_logs (not visitors) and checkin_time (not check_in_time)

create or replace function
get_recent_activity(p_property_id uuid, p_limit int default 20)
returns table (
  id uuid,
  actor_name text,
  actor_role text,
