/**
 * dashboardData.ts — Demo content for the property dashboard tiles + DetailModal.
 *
 * Responsibility: For each tile (tickets/checklist/health/energy) provides the
 *   header metrics, 7-point chart series, breakdown rows, and the AI analysis
 *   blurb shown in the expanded modal.
 * Used by: routes/dashboard.$propertyId.tsx (tile previews + modal payload),
 *   components/DetailModal.tsx (consumes TileDetail).
 * Related: lib/properties.ts (the property the tiles describe).
 *
 * Gotcha: `chartColor` is an oklch CSS string and is fed directly into Recharts
 *   stops + axis tints — keep that format. New tiles must add an entry keyed by
 *   the tile id used in dashboard.$propertyId.tsx.
 */
