/**
 * DetailModal.tsx — Expanded drill-down view for any dashboard tile.
 *
 * Responsibility: Render the metric strip, a Recharts area chart, the
 *   breakdown rows, and the AI Analysis blurb for a given TileDetail. Closes
 *   when the user taps the backdrop or the X button.
 * Used by: routes/dashboard.$propertyId.tsx (passes the active tile detail).
 * Related: lib/dashboardData.ts (TileDetail shape + content).
 *
 * Gotcha: Open state is driven by `detail !== null` rather than a boolean —
 *   passing `null` closes the modal and AnimatePresence handles the exit.
 */
