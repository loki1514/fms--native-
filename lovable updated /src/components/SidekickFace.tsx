/**
 * SidekickFace.tsx — Animated white "Cassandra" assistant orb with face.
 *
 * Responsibility: Render a glowing orb with two eyes + a mouth that morphs
 *   based on FaceState. Self-blinks on a randomized interval. Acts as both a
 *   compact bottom-nav button and a large in-chat hero.
 * Used by: dashboard.$propertyId (compact in nav), SidekickChat (large state).
 *
 * Gotcha: When `state` prop is omitted, the component owns its own state and
 *   cycles idle → listening → speaking on click. When provided, parent owns it.
 *   Inner content counter-rotates so the face stays upright while the orb spins.
 */
