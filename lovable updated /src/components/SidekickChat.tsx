/**
 * SidekickChat.tsx — Bottom-sheet chat surface for the Cassandra assistant.
 *
 * Responsibility: Modal sheet with the SidekickFace, suggested prompts,
 *   attach/mention/voice/send controls, an inline member picker, and a list
 *   of "attached" docs. All actions are mocked (no backend yet).
 * Used by: routes/dashboard.$propertyId.tsx (opened from the nav orb).
 * Related: SidekickFace (the avatar), lib/properties.ts (context for prompts).
 *
 * Gotcha: TEAM and SUGGESTED are hard-coded demo data — replace when wiring
 *   real members + AI suggestions. The sheet swallows backdrop clicks via
 *   stopPropagation so taps inside the panel don't dismiss it.
 */
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {/* Capabilities — quick "what I can do" card so first-time users
                understand the assistant's scope before typing. */}
            <div className="px-5 pb-3">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-3 backdrop-blur-xl">
                <div className="mb-2 flex items-center gap-1.5 text-[11px] uppercase tracking-wider text-white/55">
                  <Sparkles className="h-3 w-3" /> What I can do
                </div>
                <div className="grid grid-cols-2 gap-1.5 text-xs text-white/85">
                  <div className="flex items-center gap-1.5">
                    <Activity className="h-3.5 w-3.5 text-[oklch(0.78_0.18_22)]" />
                    Triage tickets
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Zap className="h-3.5 w-3.5 text-[oklch(0.85_0.18_85)]" />
                    Explain energy spikes
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Users className="h-3.5 w-3.5 text-[oklch(0.78_0.2_145)]" />
                    Find on-call staff
                  </div>
                  <div className="flex items-center gap-1.5">
                    <FileText className="h-3.5 w-3.5 text-[oklch(0.75_0.15_235)]" />
                    Summarise reports
                  </div>
                </div>
              </div>
